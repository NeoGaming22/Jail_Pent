...
AddEventHandler("poke_adminjail:jail_player", function(time)
    ...
    for k, v in pairs(Config.Jails) do
        if not jailed then
            ...
            jail_time = time
            jailed = true
            TriggerEvent("vorp:TipBottom", 'Jouduit vankilaan '..time_minutes..' minuutiksi', 5000)
            Citizen.Wait(1000)
...
AddEventHandler("poke_adminjail:unjail_player", function()
    ...
    TriggerEvent("vorp:TipBottom", 'Pääsit pois vankilasta, ole ihmisiksi', 5000)
    jailed = false
    jail_time = nil
...
Citizen.CreateThread(function ()
    while true do
        if jailed then
            DrawTxt('VANKILA: '..jail_time..' sekuntia jäljellä', 0.3, 0.95, 0.4, 0.4, true, 255, 255, 255, 150, false)
        end
        Citizen.Wait(0)
    end
end)
